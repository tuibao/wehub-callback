...
注:上述action中,回调接口必须实现对login的正确处理,否则使用相应appid的wehub 客户端将无法使用,对于其他不感兴趣/不想处理的action,可简单返回一个空的json{}.(总之收到wehub发送request后,回调接口必须有respone返回, 原因见[Faq](/main/faq.html#faq12)
...
[websocket为什么需要心跳?](https://blog.csdn.net/feiwutudou/article/details/80564630)
...
```
<div>双方仍约定采用json格式utf-8编码的文本进行通讯(所有的数据格式仍然和目前已有的格式保持一致)
...
更多的问题请参考<a href="faq.html">faq</a>